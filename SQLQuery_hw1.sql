...
SELECT 'Name:' + Name + ' -- Color:' + Color "Name with Color"
FROM Production.Product
WHERE Color IS NOT NULL
...
--7.Write a query to retrieve the to the columns ProductID and Name from the Production.
--Product table filtered by ProductID from 400 to 500 using between
...
--9. Write a query to get a result set on products that beginswith the letter S. 
SELECT ProductID, Name, Color
FROM Production.Product
WHERE Name LIKE 's%'
